...
def create_group(compute, project,group_size,zone, name, bucket):
    ...
    region = compute.regions().get(project = project , zone = zone[:len(zone)-2]).execute()
    ...
    config = { # Represents an Instance Group resource.
                ...
                "size": int(group_size), # [Output Only] The total number of instances in the instance group.
...
def add_instances(compute,project, zone, name,size,template_name):
    l = []
    for number in range(1,size+1):
        l.append({"instance": template_name + str(number)})

    config ={
        "instances":l
    }
    return compute.instanceGroups().add_instances(project=project, zone = zone, instanceGroup=name , body = config).execute()
...
def main(project, bucket, zone, instance_templates, instance_templates_size , group_name, group_size,wait=True):
    ...
    operation = add_instances(compute,project,zone,group_name,instance_templates_size,template_name)
